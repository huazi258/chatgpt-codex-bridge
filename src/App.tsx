...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([refreshModules(), refreshPairing(), refreshRecoveryMessages()])
  ...
  useEffect(() => {
    ...
    void listen<BridgeStatus>('chatgpt-status', (event) => {
      setBridge(event.payload); void refreshPairing().catch(() => undefined); void refreshMessages().catch(() => undefined); void refreshRecoveryMessages().catch(() => undefined);
    ...
    void listen<{ type: string; reason?: string }>('relay-control', (event) => {
      ...
      void refreshModules().catch(() => undefined); void refreshMessages().catch(() => undefined); void refreshRecoveryMessages().catch(() => undefined);
  ...
  async function retryUnknownMessage(messageId: string) {
    ...
    try {
      ...
      await Promise.all([refreshMessages(selectedId), refreshRecoveryMessages(), refreshModules()]);
