...
export default function App() {
  ...
  useEffect(() => {
    if (selected?.phase === 'WAITING_FOR_ACCEPTANCE' && shownAcceptance.current !== selected.id) { shownAcceptance.current = selected.id; setAcceptanceOpen(true); setAcceptanceFeedback(false); setInterventionCollapsed(false); }
    if (selected?.phase !== 'WAITING_FOR_ACCEPTANCE') shownAcceptance.current = null;
  }, [selected?.id, selected?.phase]);
  useEffect(() => {
    if (selected?.phase === 'BLOCKED' && shownBlocked.current !== selected.id) { shownBlocked.current = selected.id; setBlockedOpen(true); setBlockedFeedback(''); setInterventionCollapsed(false); }
  ...
  return <main className="relay-app">
    ...
    <div className="relay-main"><TopBar selected={selected} snapshot={snapshot} pairing={pairing} bridge={bridge} canTerminate={canTerminate} stopping={busy} onConnectionDetails={() => setConnectionOpen(true)} onOpenHumanIntervention={() => { if (selected?.phase === 'WAITING_FOR_ACCEPTANCE') { setAcceptanceOpen(true); setInterventionCollapsed(false); } if (selected?.phase === 'BLOCKED') { setBlockedOpen(true); setInterventionCollapsed(false); } }} onTerminate={() => setStopConfirm(true)} />
    ...
    {selected && !creating ? intervention ? <HumanInterventionDrawer kind={intervention} collapsed={interventionCollapsed} busy={busy} blockedReason={blockedReason} feedback={intervention === 'blocked' ? blockedFeedback : feedback} recoveryMessages={selectedRecoveryMessages} acceptanceFeedbackOpen={acceptanceFeedback} onCollapsedChange={setInterventionCollapsed} onClose={() => intervention === 'blocked' ? setBlockedOpen(false) : setAcceptanceOpen(false)} onFeedbackChange={intervention === 'blocked' ? setBlockedFeedback : setFeedback} onSubmitBlocked={() => void submitBlockedFeedback()} onAccept={() => void accept()} onStartAcceptanceFeedback={() => setAcceptanceFeedback(true)} onBackToAcceptance={() => setAcceptanceFeedback(false)} onSubmitAcceptanceFeedback={() => void submitFeedback()} /> : <SessionInspector session={selected} cycles={cycles} snapshot={snapshot} collapsed={inspectorCollapsed} onCollapsedChange={setInspectorCollapsed} onOpenDirectory={() => void openDirectory(selected)} /> : null}
