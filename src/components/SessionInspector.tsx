...
export function SessionInspector({ session, cycles, snapshot, collapsed, onCollapsedChange, onOpenDirectory }: SessionInspectorProps) {
  ...
  return <aside className={`session-inspector ${collapsed ? 'collapsed' : ''}`}>
    ...
    {!collapsed ? <div className="inspector-content">
      <section><h3>状态</h3><dl><dt>当前状态</dt><dd>{phaseLabel(session.phase)}</dd><dt>Cycle</dt><dd>{session.startedCycles} / {session.maxCycles}</dd><dt>Runtime</dt><dd>{session.maxRuntimeMinutes} 分钟上限</dd><dt>Invalid replies</dt><dd>{session.invalidReplyCount}</dd></dl></section>
