...
While paired, the extension sends a local keepalive every 15 seconds and reconnects after an unexpected local bridge disconnect. Before dispatching a protocol task, the background worker verifies that the selected ChatGPT tab has adapter version `0.6.0` and injects it when required. It also returns the extracted protocol JSON through a structured bridge field, so desktop validation does not depend on ChatGPT's rendered Markdown fences. Use **Check current adapter** before protocol dispatch; it confirms both versions and whether the latest visible reply yielded structured JSON. Reload the extension and the ChatGPT tab once after upgrading to this version.
