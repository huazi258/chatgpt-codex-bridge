...
## Virtual-list reuse repair

The next Task 7 attempt produced a complete `NEXT_TASK` JSON block in ChatGPT, yet the adapter timed out with `pendingProtocolCandidate=false`. Its diagnostic reported parseable JSON in the visible assistant nodes but no qualifying DOM mutation, showing that the virtualized conversation had reused or replaced message nodes outside the mutation baseline. Adapter `1.2.0` additionally records the pre-send set of parsed protocol JSON objects. It accepts a parsed reply if it belongs to a changed node or if its JSON is absent from that pre-send set. This preserves protection against replaying an unchanged old reply while accepting a genuinely new protocol response after DOM reuse. `protocol-text.test.mjs` reproduces this exact baseline-difference decision.
